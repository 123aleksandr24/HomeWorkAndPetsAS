...
public class Server {
    ...
    private Consumer<Pair<String,String>> onMessageReceived; // колл бэк
    ...
    public void connect() {
        ...
        URI address = null;
        try {
            address = new URI ("ws://35.214:8881");
        ...
        client = new WebSocketClient(address) {
            ...
            public void onMessage(String json) {
                ...
                if (type == Protocol.MESSAGE) {
                    // пришло воходящще сообщение
                    displayIncomingMessage(Protocol.unpackMessage(json));
                }
                if (type == Protocol.USER_STATUS) {
                    //пришё статус пользователя
                    Protocol.unpackStatus(json);
                    }
    ...
    public void sendName (String name) {
        ...
        if (client != null && client.isOpen() ) {
            client.send(Protocol.packName(usrName));
        }
    }
    ...
    private void updateStatus (Protocol.UserStatus status) {
        // Запомнить что такой-0то пользователь (имя ID)
        // имеет такой-то статус (онлайн или офлайн)
    ...
    private void displayIncomingMessage (Protocol.Message message) {
        ...
        onMessageReceived.accept(
                new Pair<>(name,message.getEncodedText()) // отправляем в мэйнактивити пришедшее сообщение
        );
    }
